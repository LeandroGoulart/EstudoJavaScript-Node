...
Problema
Segundo o Instituto de Astronomia, Geofísica e Ciências Atmosféricas (IAG) da Universidade de São Paulo (USP),
a atmosfera, camada de gás que envolve nosso planeta, é constituída de cinco camadas: 
...
É por isso que os alpinistas normalmente levam oxigênio com eles quando escalam altas montanhas
 (começa a "faltar" ar para a respiração). A troposfera é a única camada em que os seres vivos podem respirar normalmente.
...
Você está retornando um número quando deveria retornar uma string ou vice-versa. Lembre-se que o retorno deve ser exatamente o que é pedido no exercício.
*/
function identificarCamadaAtmosfera(altitude) {
    if (altitude <= 20) {
      return "TROPOSFERA";
    } else if (altitude <= 50) {
      return "ESTRATOSFERA";
